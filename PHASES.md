...
### Phase 2y — Workspace → Class → Group → Channel ✅ Built and verified against MySQL

**Objective:** the user's finalized org model. Two levels inserted between Workspace and Channel,
with a three-pane UI that makes position in the hierarchy unambiguous.

| Level | Model | Table | Scope of its slug |
|---|---|---|---|
| Workspace | `Workspace` | `workspaces` | global |
| Class | `WorkspaceClass` | `workspace_classes` | per workspace |
| Group | `Group` | `workspace_groups` | per class |
| Channel | `Team` (unchanged) | `teams` | per workspace |

**Two naming collisions, both forced:** `class` is a Python keyword, so the model is
`WorkspaceClass`. `groups` is a **MySQL 8 reserved word** (the window-function frame clause) — the
first migration attempt died on `INSERT INTO groups`, so the table is `workspace_groups`. Neither
is cosmetic; both were discovered by failing, not by guessing.

**Channel is still `Team`.** Renaming that table would have touched every module reading `team_id`
for zero behavioral gain. The name stayed; the position in the hierarchy changed.

### The one invariant worth stating

`Team.workspace_id` is **denormalized** — reachable via `group → class → workspace`. It stays
because ~10 modules already authorize against it (`require_channel_role`, `channel_briefing`,
`channel_readiness`, connection assignment), and rewriting the authorization layer into three-table
joins to remove one column is a bad trade.

The risk of denormalization is *divergence*: a Channel claiming workspace A while its Group's Class
belongs to workspace B would make every one of those checks ask about the wrong tenant. So the
write path never accepts `workspace_id` from a caller — it derives it from the parent Group, and
`test_channel_belongs_to_exactly_one_group_and_derives_its_workspace` pins that the two are the
same value.

### Isolation is structural, not defensive

Every lookup is scoped by its parent: `get_class_in_workspace`, `get_group_in_class`,
`get_group_in_workspace`. **The fetch-by-bare-id variant is not offered**, because fetching then
checking is identical to fetching until someone forgets the second half. A Group id from another
workspace doesn't fail an ownership check — it produces no row.

Route shapes mirror this: `/workspaces/{id}/classes/{id}/groups/{id}`. The parent in the path *is*
the authorization scope. Cross-boundary ids return **404, not 403** — "exists, but not yours"
confirms another tenant's org chart.

| RBAC | Who |
|---|---|
| Create/edit/delete Class | workspace admins (`super_admin`, `org_admin`) |
| Create/edit/delete Group | those + `team_manager` (team structure is that role's job) |
| Create Channel | unchanged |
| Read the hierarchy | any workspace member — you can't navigate to a channel inside a group whose name you can't see |

**`delete_class` and `delete_group` refuse while non-empty** rather than cascading. The blast radius
of a cascade here is a team's channels and with them their connection assignments, requirements and
AI history — two levels below what the admin clicked.

### Migration and backfill

Hand-written, not autogenerated: `teams.group_id` must end NOT NULL, but existing channels had no
Group. Column added nullable → existing channels adopted into a `General` class with groups named
after their old free-text `category` → tightened to NOT NULL. That column was the closest thing to
a grouping the old schema had, so honoring it preserves whatever structure an admin had expressed.
Personal workspaces are skipped; a surviving orphan raises with its ids rather than being given an
invented home.

### The UI: three panes, one per question

    [rail]       which workspace am I in?   (context switcher: Personal vs each org)
    [tree]       where in it?               (Class > Group > Channel, collapsible)
    [work area]  what am I doing here?      (breadcrumb + one module)

The previous single sidebar mixed workspace switching, channel listing and workspace nav in one
column — exactly the "one cluttered surface for four levels" this replaces. `Sidebar.tsx` is
deleted; its invite entry points moved to the levels they act on (workspace invite in the tree
header, channel invite in the channel's Members module).

**Channel modules are routes, not tabs.** `/channels/:teamId/:module` — only the chosen module
mounts, so opening a channel costs one request per pane instead of nine, and each module refetches
on mount rather than rendering cached state while connections keep syncing.

**Extensions is the setup gateway.** It shows `2/3 complete`, the module nav carries a count badge,
and every other module shows a "setup incomplete" banner rather than an empty state that reads as
"nothing happening".
...
| Module | State |
|---|---|
| Sentinel, Attention, Extensions, Members, Settings | real, wired to existing backends |
| **Feed** | **built this phase** — `channel_feed.py`, a projection of Signals through the existing `_channel_scope` (no new pipeline, no second copy of the visibility rule) |
| Prepare Me, Insights, Knowledge | **listed and marked "soon"** — no backend exists |

The unbuilt three render `NotBuiltModule`, which says so. An empty Insights page and an unbuilt
Insights page look identical to a user, and only one of them is honest.

### Verified

29 new isolation tests: Workspace→Class, Class→Group, Group→Channel ownership; cross-workspace,
cross-class, cross-group, cross-channel isolation; RBAC per level; Personal-workspace privacy; and
that Phase A/B per-user connection isolation survives the new levels. **Suite: 236 passed.**
Frontend `tsc -b && vite build` clean.

MySQL probes: workspace derived correctly, cross-tenant channel creation refused, breadcrumb
resolves, duplicate group slug rejected per class but allowed across classes, backfill placed both
existing channels, and the spec's own example tree (Development → Backend/Frontend Team →
#api-development, #database, #web) builds and reads back through the route layer.

### Known gaps (deliberately deferred, not oversights)

- **No Class or Group detail pages.** Both are creatable and navigable from the tree, but there's no
  screen for renaming a class or reordering groups — the API supports both (`PATCH`, `position`).
- **No drag-to-reorder and no moving a channel between groups.** `position` exists on the models;
  nothing writes it but creation order.
- **Feed doesn't paginate or poll.** It refetches on mount and on an explicit Refresh; 50 items,
  no cursor.
- **The old `Team.category` column still exists**, now vestigial. Left in place rather than dropped
  in the same migration that depends on reading it.

**Exit criteria:** the hierarchy is enforced in the database and the authorization layer, the UI
communicates position at every level without mixing controls across levels, and cross-boundary
access is structurally impossible. **Confirmed against MySQL.**
