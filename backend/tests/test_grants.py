...
def test_microsoft_grant_provisions_its_services_via_the_same_path(session, env):
    ...
    assert _providers(session, env["ws"].id, env["a"].id) == ["microsoft_outlook_calendar", "microsoft_outlook_mail"]
    ...
    assert {c.repo for c in conns} == {"mail", "calendar"}
...
def test_reprovision_same_account_is_idempotent_and_clears_revocation(session, env):
    ...
    conns = session.execute(select(Connection).where(Connection.user_id == env["a"].id)).scalars().all()
    assert len(conns) == 2  # not duplicated
...
def test_grant_is_scoped_per_user(session, env):
    ...
    # Two members, no collision - each owns their own two connections.
    assert len(_providers(session, env["ws"].id, env["a"].id)) == 2
    assert len(_providers(session, env["ws"].id, env["b"].id)) == 2
    assert session.execute(select(Connection)).scalars().all().__len__() == 4
