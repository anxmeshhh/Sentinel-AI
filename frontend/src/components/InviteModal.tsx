      ...
      {!invite ? (
        ...
        <>
          <div className="mb-3 flex items-center gap-2 border border-border bg-ground px-3 py-2.5">
            <span className="min-w-0 flex-1 truncate text-[12.5px] text-ink-dim">{link}</span>
            <button onClick={copyLink} className="flex-none text-[12px] font-semibold text-ink underline underline-offset-2">
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
          <p className="text-[11.5px] text-ink-faint">No expiry, unlimited uses — revoke support coming later.</p>
        </>
      )}
    </Modal>
  );
